...
_TOOL_GROUPS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("SECURITY TOOLS — is this machine clean, what is behind that domain, is this email real",
     ("forensics", "security", "recon", "email")),
...
_TOOL_EXAMPLES = {
    "process": "ti auto what is running on port 8080",
    ...
    "recon": "ti auto what is behind example.com",
    "email": "ti auto is this email phishing suspicious.eml",
...
TOPICS = {
    ...
    "recon": ("TI RECON — WHAT IS BEHIND A DOMAIN OR AN ADDRESS", (
        "Name a domain or a public address and TACU gathers what can be known about it from the",
        "outside: the names beneath it from certificate transparency and DNS, what each resolves",
        "to, the certificate it presents, the CDN or WAF in front of it, the network that owns the",
        "address, where it sits, and what reputation sources report. Then a score that shows its",
        "working. Every answer ends with the services that were contacted.",
        "",
        "Two rules hold throughout. One indicator is never a finding: Server: cloudflare on its own",
        "is 40%, and a CNAME, three headers, a cookie and the ASN agreeing is 98% — each provider",
        "match lists the indicators behind it. And a reputation source is evidence, never a verdict:",
        "an address behind a CDN hosts thousands of unrelated customers, so the raw findings are",
        "kept and the score lists every contribution, including the ones that lower it.",
        "",
        "Everything at once:",
        "  ti auto what is behind example.com",
        "  ti auto recon on example.com",
        "  ti auto how risky is 203.0.113.10",
        "One question:",
        "  ti auto list subdomains of example.com",
        "  ti auto is example.com behind cloudflare        # WAF/CDN, scored across indicators",
        "  ti auto certificate for example.com             # issuer, SANs, validity, fingerprint",
        "  ti auto dns records for example.com             # A/AAAA, CNAME chain, NS, MX, TXT, PTR",
        "  ti auto who owns the ip 203.0.113.10            # origin AS, prefix, registry, allocation",
        "  ti auto where is 203.0.113.10 located           # country, region, city, organisation",
        "  ti auto is 203.0.113.10 malicious               # AbuseIPDB + VirusTotal, raw",
        "Raw contract:",
        "  ti tools run recon --input '{\"operation\":\"sweep\",\"target\":\"example.com\"}'",
        "  operations: sweep, subdomains, dns, tls, http, waf, asn, geo, reputation",
        "  options: limit (subdomains, up to 200), bruteforce (try common names by DNS; off by",
        "  default), timeout (seconds per request)",
        "",
        "Reputation keys:",
        "  AbuseIPDB and VirusTotal need an API key each. Without one the answer says",
        "  \"not checked\" rather than guessing. Store them once, never on a command line:",
        "  ti config keys                     # prompts for each, never echoed, saved with mode 0600",
        "  or for scripts: TACU_ABUSEIPDB_KEY and TACU_VIRUSTOTAL_KEY in the environment",
        "",
        "What this is not:",
        "  A question about this machine's own connections is network or forensics, not recon.",
        "  A name that resolves to a private or loopback address is reported and never probed,",
        "  so recon cannot be pointed at something inside your network.",
        "  Discovery is passive: certificate logs and DNS. bruteforce tries a short list of common",
        "  names by DNS only, and only when asked. Use it on domains you are authorised to test.",
        "",
        "Sources, and what each costs:",
        "  crt.sh (certificate transparency)   no key · sometimes answers empty; TACU retries once",
        "  dig / your resolver                  no key · records the exact command it ran",
        "  asn.cymru.com over DNS               no key · origin AS, prefix, registry, allocation date",
        "  ipinfo.io                            no key · geolocation, keyless tier",
        "  AbuseIPDB, VirusTotal                key    · reputation; both named as not checked without one",
    )),
    "email": ("TI EMAIL — IS THIS MESSAGE WHAT IT CLAIMS TO BE", (
        "Save the message as an .eml and name it. TACU reads what it says about itself — the",
        "headers, the path it claims to have taken, who authenticated for which identity, every",
        "link and address it carries, and what each attachment actually is — and scores it in the",
        "open, every reason listed. Nothing in the message is ever executed, rendered, visited or",
        "connected to. Links are shown defanged. Attachments are typed by their first bytes and",
        "hashed in memory; archives are inventoried from their directory and never opened.",
        "",
        "The finding that matters most is alignment. \"SPF pass\" is not reassurance until you know",
        "for whom: a spoof usually passes SPF and DKIM for the attacker's own domain while the From",
        "shows yours, and DMARC is what fails. The authentication block says for whom each passed.",
        "",
        "Everything at once:",
        "  ti auto is this email phishing suspicious.eml",
        "  ti auto analyze invoice.eml",
        "One question:",
        "  ti auto check the headers of suspicious.eml       # anomalies and authentication",
        ...
        "  ti auto extract iocs from suspicious.eml           # URLs, domains, IPs, addresses, hashes — defanged",
        "  ti auto is the attachment in suspicious.eml safe   # magic type, mismatch, archive inventory",
        "Raw contract:",
        "  ti tools run email --input '{\"operation\":\"analyze\",\"path\":\"suspicious.eml\"}'",
        "  operations: analyze, headers, auth, hops, iocs, attachments",
        "  options: enrich (passive lookups; on for analyze and hops), timeout (seconds per request)",
        "",
        "Scoring:",
        "  DMARC fail +30 · DKIM invalid +20 · SPF fail +15 · Reply-To differs +15 · display-name",
        "  impersonation +15 · new, lookalike or unregistered sender domain +20 · malicious URL +30",
        "  · bad sender IP +25 · executable attachment +40 · extension/content mismatch +25 ·",
        "  suspicious archive +20 · auth passed for another identity +20 · link text lies +15 ·",
        "  link to a bare IP +10 · no DMARC record +5. HIGH at 60, MEDIUM at 30.",
        "",
        "What goes out, and what never does:",
        "  Parsing is entirely local. With enrich on, TACU asks third parties *about* what the mail",
        "  names — PTR, ASN and reputation for hop addresses, MX/SPF/DMARC/DKIM records and whois",
        "  age for the sender domain, VirusTotal for domains and attachment hashes when a key is",
        "  set — and never sends a packet to any host, URL or address the mail itself contains.",
        "  Reputation keys: ti config keys. Without one the answer says \"not checked\".",
    )),
    "processes": ("TI PROCESSES — WHAT IS RUNNING, AND WHAT IS IT", (
        "TACU reads a command line the way an engineer does — the interpreter, the module or",
        "script it was pointed at, the package-manager subcommand — and checks what the process",
        "holds. A listening socket is what makes something a server, whatever it is called, so a",
        "tool released next year is described as accurately as a familiar one.",
        "Ports come from the socket table, never from the command line, because a process can be",
        "told one port and bind another.",
        "",
        "Find something:",
        "  ti auto what is running on port 8080",
        "  ti auto which process is running my python http server",
        "  ti auto is vite running",
        "  ti auto what servers are running on this machine",
        "Understand one:",
        "  ti auto tell me all about process with id 92894",
        "  ti auto show me the process tree for pid 92894",
        "Rankings and listings are unchanged:",
        "  ti auto which process is consuming most CPU",
        "  ti auto list running processes",
        "Raw contract:",
        "  ti tools run process --input '{\"operation\":\"graph\",\"port\":8080}'",
        "  filters: port, pid, query; with none, it shows what is listening.",
        "",
        "Answers name the PID and how to stop it, because that is usually the next step.",
        "Another user's processes need sudo; see ti help forensics.",
    )),
...
ALIASES = {
    ...
    "reconnaissance": "recon", "subdomains": "recon", "subdomain": "recon", "waf": "recon",
    "cdn": "recon", "reputation": "recon", "asn": "recon",
    "eml": "email", "phishing": "email", "phish": "email", "mail": "email", "dkim": "email",
    "dmarc": "email", "spf": "email",
}
